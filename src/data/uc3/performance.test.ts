...
const KPI_BODY = {
  ...
  metrics: {
    total_teus: 18432.5, total_tonnes: 241900, vessel_calls: 17,
    yard_occupancy_pct: 71.4, gate_total_teus: 9100, gate_in_teus: 4400,
    gate_out_teus: 4700, total_pendency_teus: 1320,
    reefer_available_slots: 210, reefer_total_slots: 900,
  },
  deltas: { total_teus: 512.5, total_pendency_teus: -80, vessel_calls: 0 },
};

const TRAFFIC: PerformanceTrafficWire = {
  report_date: '2026-07-25', terminal_code: 'NSICT', period: 'DAY', vessels: 3,
  imp_teus: 2100, exp_teus: 1980, total_teus: 4080, rakes: 4,
  rail_dis_teus: 640, rail_ldg_teus: 610, rail_total_teus: 1250,
};

const env = <T,>(items: T[], total = items.length) =>
  ({ items, total, limit: 50, offset: 0, count: items.length });
...
const stub = (body: unknown, status = 200, statusText = 'OK') =>
  vi.fn((url: string, _init?: RequestInit) =>
    String(url).endsWith('/auth/login') ? jsonResponse(loginBody) : jsonResponse(body, status, statusText));
...
describe('mapPerformanceMetrics', () => {
  it('maps every headline metric', () => {
    ...
    expect(m).toEqual({
      totalTeus: 18432.5, totalTonnes: 241900, vesselCalls: 17,
      yardOccupancyPct: 71.4, gateTotalTeus: 9100, gateInTeus: 4400,
      gateOutTeus: 4700, totalPendencyTeus: 1320,
      reeferAvailableSlots: 210, reeferTotalSlots: 900,
    });
  });

  it('yields null — never 0 — for a section the report omits', () => {
    const m = mapPerformanceMetrics({ ...KPI_BODY.metrics, yard_occupancy_pct: null, total_tonnes: null });
...
describe('mapPerformanceTraffic', () => {
  it('maps every column and builds a composite key', () => {
    ...
    expect(r).toMatchObject({
      reportDate: '2026-07-25', terminalCode: 'NSICT', period: 'DAY',
      vessels: 3, impTeus: 2100, expTeus: 1980, totalTeus: 4080,
      rakes: 4, railDisTeus: 640, railLdgTeus: 610, railTotalTeus: 1250,
    });
  });

  it('still yields a unique key when the row carries no identifying fields', () => {
    const a = mapPerformanceTraffic({ ...TRAFFIC, report_date: null, terminal_code: null, period: null }, 0);
    const b = mapPerformanceTraffic({ ...TRAFFIC, report_date: null, terminal_code: null, period: null }, 1);
...
describe('parsePerformanceTrafficPage', () => {
  it('preserves server order and passes total through', () => {
    const p = parsePerformanceTrafficPage(
      env([TRAFFIC, { ...TRAFFIC, terminal_code: 'GTI' }], 812), 25);
...
describe('mapPerformanceTerminal', () => {
  it('falls back to the code when full_name is absent', () => {
    const t = mapPerformanceTerminal({
      code: 'BMCT', full_name: null, operator: 'JSW', terminal_type: 'CONTAINER',
      is_container: true, sort_order: 2,
    })!;
  ...
  it('drops a row with no code — it cannot be a filter value', () => {
    expect(mapPerformanceTerminal({
      code: null, full_name: 'x', operator: null, terminal_type: null,
      is_container: null, sort_order: null,
    })).toBeNull();
  });
...
describe('query builders — only parameters the gateway accepts', () => {
  ...
  it('traffic: emits terminal / period / sort / direction and omits blanks', () => {
    const q = performanceTrafficQuery({ terminal: 'NSICT', period: 'DAY', sort: 'total_teus', direction: 'asc' });
...
describe('fetch* (end to end over a stubbed transport)', () => {
  ...
  it('terminals: maps the dimension list', async () => {
    vi.stubGlobal('fetch', stub(env([{
      code: 'NSICT', full_name: 'Nhava Sheva International Container Terminal',
      operator: 'DP World', terminal_type: 'CONTAINER', is_container: true, sort_order: 1,
    }])));
