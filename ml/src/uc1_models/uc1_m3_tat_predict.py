...
if _HAS_FASTAPI:
    ...
    # every request. Keyed by (engine, seed, n_days).
    ...
    def build_router() -> "APIRouter":
        ...
        def predict(req: TATPredictRequest) -> Dict[str, Any]:
            predictor = _cached_predictor(req.engine)
            return predictor.predict(req.to_features()).as_dict()
        ...
        def predict_batch(reqs: List[TATPredictRequest]) -> List[Dict[str, Any]]:
            ...
            out = []
            for r in reqs:
                out.append(_cached_predictor(r.engine).predict(r.to_features()).as_dict())
            return out
